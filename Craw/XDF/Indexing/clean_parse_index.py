    ...
    try:
        ...
        for part in reading:
            ...
            for j in questions:
                ...
                # print(url)
                insert_sql = """INSERT INTO xdf.reading (url, examid) VALUES (%s, %s)"""
        ...
        for part in listening:
            ...
            if len(questions) == 5:  # conversation
                for i in questions:
                    ...
                    # print(url)
                    insert_sql = """INSERT INTO xdf.listening_conversation (url, examid) VALUES (%s, %s)"""
                    cur.execute(insert_sql, (url, examid,))
            elif len(questions) == 6:  # lecture
                for i in questions:
                    url = "https://www.testgts.com" + i['data-url'].replace('&amp;', '&')
                    # print(url)
                    insert_sql = """INSERT INTO xdf.listening_lecture (url, examid) VALUES (%s, %s)"""
                    cur.execute(insert_sql, (url, examid,))
            else:
        ...
        if len(speaking) == 4:  # new TOEFL
            ...
            q1_insert_sql = """INSERT INTO xdf.speaking_q1 (url, examid) VALUES (%s, %s)"""
            cur.execute(q1_insert_sql, (url, examid,))

            q2 = "https://www.testgts.com" + speaking[1]['data-url'].replace('&amp;', '&')
            q2_insert_sql = """INSERT INTO xdf.speaking_q2 (url, examid) VALUES (%s, %s)"""
            cur.execute(q2_insert_sql, (url, examid,))

            q3 = "https://www.testgts.com" + speaking[2]['data-url'].replace('&amp;', '&')
            q3_insert_sql = """INSERT INTO xdf.speaking_q3 (url, examid) VALUES (%s, %s)"""
            cur.execute(q3_insert_sql, (url, examid,))

            q4 = "https://www.testgts.com" + speaking[3]['data-url'].replace('&amp;', '&')
            q4_insert_sql = """INSERT INTO xdf.speaking_q4 (url, examid) VALUES (%s, %s)"""
            cur.execute(q4_insert_sql, (url, examid,))
        ...
        if len(writing) != 2:
            ...
            # print(integrated)
            integrated_insert_sql = """INSERT INTO xdf.writing_integrated (url, examid) VALUES (%s, %s)"""
            cur.execute(integrated_insert_sql, (url, examid,))

            independent = "https://www.testgts.com" + writing[1]['data-url'].replace('&amp;', '&')
            # print(independent)
            independent_insert_sql = """INSERT INTO xdf.writing_independent (url, examid) VALUES (%s, %s)"""
            cur.execute(independent_insert_sql, (url, examid,))

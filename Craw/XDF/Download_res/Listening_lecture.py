...
cur = conn.cursor()
cur.execute(
    "SELECT DISTINCT examid, audioid, listening_audio_url FROM tpo.xdf.listening_lecture WHERE listening_audio_downloaded is null ORDER BY examid, audioid")
rows = cur.fetchall()

download_dir = '/Users/Taylor/Desktop/MyTPO/Craw/XDF/Download_res/res/Listening_lecture_audio'
        ...
        with requests.get(audio_url, stream=True) as r:
            ...
            with open(file_path, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
        cur.execute(
            "UPDATE tpo.xdf.listening_lecture SET listening_audio_local_name = %s, listening_audio_downloaded = true WHERE listening_audio_url = %s",

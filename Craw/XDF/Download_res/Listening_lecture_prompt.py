...
cur = conn.cursor()
cur.execute(
    "SELECT DISTINCT examid, audioid, num, prompt_audio_url FROM tpo.xdf.listening_lecture WHERE prompt_audio_download is null ORDER BY examid, audioid")
rows = cur.fetchall()

download_dir = '/Users/Taylor/Desktop/MyTPO/Craw/XDF/Download_res/res/Listening_lecture_prompt'
        ...
        with requests.get(audio_url, stream=True) as r:
            ...
            with open(file_path, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
        cur.execute(
            "UPDATE tpo.xdf.listening_lecture SET prompt_audio_local_name = %s, prompt_audio_download = true WHERE prompt_audio_url = %s",

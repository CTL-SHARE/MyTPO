...
cur = conn.cursor()
cur.execute("SELECT url, examid, audio_url FROM tpo.xdf.speaking_q2 WHERE audio_download = false ORDER BY examid")
rows = cur.fetchall()

download_dir = '/Users/Taylor/Desktop/MyTPO/Craw/XDF/Download_res/res/Speaking_q2'
        ...
        with requests.get(audio_url, stream=True) as r:
            ...
            with open(file_path, 'wb') as f:
                for chunk in r.iter_content(chunk_size=8192):
                    f.write(chunk)
        cur.execute("UPDATE tpo.xdf.speaking_q2 SET audio_local_name = %s, audio_download = true WHERE url = %s",
                    (local_filename, url))
        conn.commit()

...
cur = conn.cursor()
cur.execute("SELECT examid, full_html FROM xdf.speaking_q2")
rows = cur.fetchall()
...
for row in rows:
    ...
    cur.execute(
        "UPDATE xdf.speaking_q2 SET caption_html = %s, caption_clean = %s, passage_html = %s, passage_clean = %s, audio_url = %s, audio_download = false, transcription = %s WHERE examid = %s",
        (

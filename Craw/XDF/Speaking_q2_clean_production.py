...
cur.execute("SELECT passage_clean, url FROM tpo.xdf.speaking_q2 where passage_json is null")
rows = cur.fetchall()
...
for row in rows:
    ...
    cur.execute("UPDATE tpo.xdf.speaking_q2 SET passage_json = %s WHERE url = %s", (str(passage_json), url))
    conn.commit()

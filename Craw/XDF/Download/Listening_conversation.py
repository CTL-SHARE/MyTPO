...
cur = conn.cursor()
cur.execute("SELECT * FROM tpo.xdf.listening_conversation WHERE downloaded = false OR downloaded IS NULL ")
rows = cur.fetchall()
...
def craw(url):
    ...
    cur.execute(
        "UPDATE tpo.xdf.listening_conversation SET question_index = %s, full_html = %s, downloaded=true WHERE url = %s",
        (idx, html, url))

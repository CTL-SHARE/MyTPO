...
cur = conn.cursor()
cur.execute("SELECT * FROM tpo.xdf.speaking_q1 WHERE downloaded = false OR downloaded IS NULL")
rows = cur.fetchall()
...
def craw(url):
    ...
    cur.execute("UPDATE tpo.xdf.speaking_q1 SET full_html = %s, downloaded=true WHERE url = %s", (html, url))

...
cur = conn.cursor()
cur.execute("SELECT * FROM tpo.xdf.reading WHERE downloaded = false OR downloaded IS NULL")
rows = cur.fetchall()
...
def craw(url):
    ...
    cur.execute("UPDATE tpo.xdf.reading SET question_index = %s, full_html = %s, downloaded=true WHERE url = %s",
                (idx, html, url))

...
cur.execute(
    "SELECT url, full_html, examid, audioid, question_index FROM tpo.xdf.listening_conversation WHERE parsed is NULL order by examid, audioid, num")
...
def parse(html):
    ...
    cur.execute(
        "UPDATE tpo.xdf.listening_conversation SET question_html = %s, prompt_clean = %s, choices = %s, answers = %s, audio_url = %s, audio_download = false, transcription = %s, parsed = true WHERE url = %s",
...
for question in questions:
    try:
        ...
        cur.execute(
            "UPDATE tpo.xdf.listening_conversation SET caption_html = %s, caption_clean = %s WHERE url = %s",
